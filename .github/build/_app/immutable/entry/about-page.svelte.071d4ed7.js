import {
	S as rt,
	i as st,
	s as ot,
	k as l,
	a as f,
	q as z,
	J as ut,
	l as r,
	h as a,
	c as m,
	m as u,
	r as M,
	n as i,
	K as W,
	p as C,
	D as e,
	b as d,
	H as Y
} from '../chunks/index.e03ef04e.js';
const ct = '' + new URL('../assets/github.d83ddf4e.png', import.meta.url).href,
	nt =
		'data:image/png;base64,',
	ht = '' + new URL('../assets/telegram.f9edfe0a.png', import.meta.url).href;
function ft(lt) {
	let p,
		E,
		D,
		S,
		V,
		w,
		X,
		y,
		k,
		q,
		J,
		o,
		g,
		c,
		F,
		U,
		v,
		O,
		j,
		b,
		n,
		N,
		B,
		T,
		H,
		x,
		A,
		h,
		Z,
		I,
		P,
		Q;
	return {
		c() {
			(p = l('meta')),
				(E = f()),
				(D = l('h2')),
				(S = z('About Me')),
				(V = f()),
				(w = l('p')),
				(X = z(`Data Scientist, Software Developer, 
        with a growing thirst for knowledge, and a never-ending passion for IT.`)),
				(y = f()),
				(k = l('h2')),
				(q = z('Contacts')),
				(J = f()),
				(o = l('ul')),
				(g = l('li')),
				(c = l('img')),
				(U = f()),
				(v = l('a')),
				(O = z('GitHub')),
				(j = f()),
				(b = l('li')),
				(n = l('img')),
				(B = f()),
				(T = l('a')),
				(H = z('Telegram')),
				(x = f()),
				(A = l('li')),
				(h = l('img')),
				(I = f()),
				(P = l('a')),
				(Q = z('Email')),
				this.h();
		},
		l(t) {
			const s = ut('svelte-1ds1qyu', document.head);
			(p = r(s, 'META', { name: !0, content: !0 })), s.forEach(a), (E = m(t)), (D = r(t, 'H2', {}));
			var _ = u(D);
			(S = M(_, 'About Me')), _.forEach(a), (V = m(t)), (w = r(t, 'P', {}));
			var $ = u(w);
			(X = M(
				$,
				`Data Scientist, Software Developer, 
        with a growing thirst for knowledge, and a never-ending passion for IT.`
			)),
				$.forEach(a),
				(y = m(t)),
				(k = r(t, 'H2', {}));
			var tt = u(k);
			(q = M(tt, 'Contacts')), tt.forEach(a), (J = m(t)), (o = r(t, 'UL', {}));
			var R = u(o);
			g = r(R, 'LI', {});
			var K = u(g);
			(c = r(K, 'IMG', { src: !0, width: !0, height: !0, style: !0, alt: !0 })),
				(U = m(K)),
				(v = r(K, 'A', { id: !0, href: !0 }));
			var et = u(v);
			(O = M(et, 'GitHub')), et.forEach(a), K.forEach(a), (j = m(R)), (b = r(R, 'LI', {}));
			var L = u(b);
			(n = r(L, 'IMG', { src: !0, width: !0, height: !0, style: !0, alt: !0 })),
				(B = m(L)),
				(T = r(L, 'A', { id: !0, href: !0 }));
			var at = u(T);
			(H = M(at, 'Telegram')), at.forEach(a), L.forEach(a), (x = m(R)), (A = r(R, 'LI', {}));
			var G = u(A);
			(h = r(G, 'IMG', { src: !0, width: !0, height: !0, style: !0, alt: !0 })),
				(I = m(G)),
				(P = r(G, 'A', { id: !0, href: !0 }));
			var it = u(P);
			(Q = M(it, 'Email')), it.forEach(a), G.forEach(a), R.forEach(a), this.h();
		},
		h() {
			(document.title = 'About'),
				i(p, 'name', 'description'),
				i(p, 'content', 'About this app'),
				W(c.src, (F = ct)) || i(c, 'src', F),
				i(c, 'width', '15'),
				i(c, 'height', '18'),
				C(c, 'vertical-align', 'bottom'),
				i(c, 'alt', 'Github'),
				i(v, 'id', 'gh-link'),
				i(v, 'href', 'https://github.com/sparrow12345'),
				W(n.src, (N = ht)) || i(n, 'src', N),
				i(n, 'width', '15'),
				i(n, 'height', '18'),
				C(n, 'vertical-align', 'bottom'),
				i(n, 'alt', 'Telegram'),
				i(T, 'id', 'tg-link'),
				i(T, 'href', 'https://t.me/Isildur_son_of_Elendil'),
				W(h.src, (Z = nt)) || i(h, 'src', Z),
				i(h, 'width', '15'),
				i(h, 'height', '18'),
				C(h, 'vertical-align', 'bottom'),
				i(h, 'alt', 'Google'),
				i(P, 'id', 'email'),
				i(P, 'href', 'mailto:majdnaser2013@gmail.com');
		},
		m(t, s) {
			e(document.head, p),
				d(t, E, s),
				d(t, D, s),
				e(D, S),
				d(t, V, s),
				d(t, w, s),
				e(w, X),
				d(t, y, s),
				d(t, k, s),
				e(k, q),
				d(t, J, s),
				d(t, o, s),
				e(o, g),
				e(g, c),
				e(g, U),
				e(g, v),
				e(v, O),
				e(o, j),
				e(o, b),
				e(b, n),
				e(b, B),
				e(b, T),
				e(T, H),
				e(o, x),
				e(o, A),
				e(A, h),
				e(A, I),
				e(A, P),
				e(P, Q);
		},
		p: Y,
		i: Y,
		o: Y,
		d(t) {
			a(p), t && a(E), t && a(D), t && a(V), t && a(w), t && a(y), t && a(k), t && a(J), t && a(o);
		}
	};
}
class pt extends rt {
	constructor(p) {
		super(), st(this, p, null, ft, ot, {});
	}
}
export { pt as default };
